...
def _load_music21():
    try:
        ...
        raise RuntimeError(
            "music21 is not installed. Install the music extra or run install.sh first."
        ) from exc
...
def _build_part(stream_mod, note_mod, chord_mod, instrument_mod, name: str, measures: int):
    ...
    for _ in range(measures):
        if name == "piano":
            chord_note = chord_mod.Chord(pitches[:3])
...
def generate_score_artifacts(request: ScoreRequest) -> dict[str, object]:
    ...
    measures = max(4, len([line for line in request.lyrics.splitlines() if line.strip()]) or 4)
    ...
    for instrument_name in instruments:
        part = _build_part(stream_mod, note_mod, chord_mod, instrument_mod, instrument_name, measures)
    ...
    results: dict[str, object] = {"status": "ok", "parts": instruments}
